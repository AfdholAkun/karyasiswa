	...
	<div class="row">
		<div class="panel panel-default">
			<div class="panel-heading" >
				<span style="font-size: 11pt">Data Instansi Asal
				</span>
			</div>
		...
		<div class="modal-content">
			...
			<form action="#" id="form" class="form-horizontal">
				<div class="modal-body">
					...
					<input type="hidden" name="nama_instansi_lama">
					<input type="hidden" name="nama_instansi_status" value="0">
					<div class="form-group">
						<label class="control-label col-md-3">Instansi</label>
						<div class="col-md-9">
							<input type="text" name="nama_instansi" class="form-control" required maxlength="100" onkeyup="CekNamaInstansi()">
							<p id="nama_instansi_info" class="control-label" style="margin-bottom: -10px;font-size: 7pt;">*Instansi tidak boleh kosong</p>
						</div>
					</div>
				</div>
...
var SaveMethod = "Tambah";
	...
	var nama_instansi_lama = $('[name=nama_instansi_lama]');
	var nama_instansi_status = $('[name=nama_instansi_status]');

	var id = $('[name=id]');
	var nama_instansi = $('[name=nama_instansi]');	
	$(document).ready(function() {
		$('#form').on("submit", function(e){
			...
			if (SaveMethod == "Tambah") {
				url = link+'instansi_asal/InsertData';
			}else{
				url = link+'instansi_asal/UpdateData';
			}
			if (nama_instansi_status.val() != 1) {
				$('[name=nama_instansi]').parent().parent().addClass('has-error');
			}else{
				$('[name=nama_instansi]').parent().parent().removeClass('has-error');
				result += '1';
			}
			if (result == "1") {
				l.start();
	...
	function ResetInputAja() {
		...
		$('#form')[0].reset();
		$('#nama_instansi_info').text("*Instansi tidak boleh kosong");		
	}

	function CekNamaInstansi() {
		delay(function(){
			$.ajax({
				url: link+'instansi_asal/CekNamaInstansi',
				type: 'post',
				data: {nama_instansi: nama_instansi.val(), nama_instansi_lama:nama_instansi_lama.val()},
				dataType: 'json',
				success: function(data){
					CekIsToken(data.notif);
					$('#nama_instansi_info').text(data.value);
					if (data.value == '*Instansi tersedia') {
						$('[name=nama_instansi_status]').val(1);
						$('[name=nama_instansi]').parent().parent().removeClass('has-error');
					}else{
						$('[name=nama_instansi_status]').val(0);
						$('[name=nama_instansi]').parent().parent().addClass('has-error');
					}
				},
				error: function(){
					notify("Error System", "Gagal Cek Nama Instansi", "error");
				}
			});
		}, 500);
	}
	...
	function InputStatusHidden(status) {
		nama_instansi_status.val(status);
	}	
	...
	function EditData(kode){
		...
		ResetInput();
		$.ajax({
			url: link+'instansi_asal/EditData/'+kode,
			type: 'post',
			dataType: 'json',
			success:function(data){
				CekIsToken(data.notif);
				if (data.notif == 1) {
					...
					id.val(data.data.id);
					nama_instansi.val(data.data.nama_instansi);
					nama_instansi_lama.val(data.data.nama_instansi);
	...
	function DeleteData(kode) {
		swal({
			title: "Hapus data ?",
			text: "Menghapus 1 data instansi",
			type: "warning",
		...
		function(isConfirm) {
			if (isConfirm) {
				setTimeout(function(){
					$.ajax({
						url:link+'instansi_asal/DeleteData/'+kode,
						type:'post',
						async: false,
						dataType:'json',
						success: function(data){
	...
	function ResetData() {
		var row ;
		$.ajax({
			url:link+'instansi_asal/CountTable',
			async:false,
			type:'post',
			dataType:'json',
			success:function(data){
		...
		if (row > 1) {
			swal({
				title: "Reset data ?",
				text: "Menghapus semua data instansi ",
				type: "warning",
				showCancelButton: true,
				confirmButtonClass: "btn-warning",
				confirmButtonText: "Ok",
			...
			function(isConfirm) {
				if (isConfirm) {
					setTimeout(function(){
						$.ajax({
							url:link+'instansi_asal/ResetData',
							type:'post',
							async:false,
							dataType:'json',
							success: function(data){
