	...
	<div class="row">
		<div class="panel panel-default">
			<div class="panel-heading" >
				<span style="font-size: 11pt">Data Jenjang Studi </span>
			</div>
		...
		<div class="modal-content">
			...
			<form action="#" id="form" class="form-horizontal">
				<div class="modal-body">
					...
					<input type="hidden" name="nama_jenjang_studi_lama">
					<input type="hidden" name="nama_jenjang_studi_status" value="0">
					<div class="form-group">
						<label class="control-label col-md-3">Jenjang Studi</label>
						<div class="col-md-9">
							<input type="text" name="nama_jenjang_studi" class="form-control" required maxlength="100" onkeyup="CekNamaJenjangStudi()">
							<p id="nama_jenjang_studi_info" class="control-label" style="margin-bottom: -10px;font-size: 7pt;">*Jenjang Studi tidak boleh kosong</p>
						</div>
					</div>
				</div>
...
<div class="modal fade" id="modal-detail">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header bg-green">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h4 class="modal-title">Detail Jenjang Studi</h4>
			</div>
			<div class="modal-body">
				<div class="table-responsive">
					<table class="table" width="100%">
						<tr>
							<th width="29%">Jenjang Studi</th>
							<th width="2%">:</th>
...
var SaveMethod = "Tambah";
	...
	var nama_jenjang_studi_lama = $('[name=nama_jenjang_studi_lama]');
	var nama_jenjang_studi_status = $('[name=nama_jenjang_studi_status]');

	var id = $('[name=id]');
	var nama_jenjang_studi = $('[name=nama_jenjang_studi]');	
	$(document).ready(function() {
		$('#form').on("submit", function(e){
			...
			if (SaveMethod == "Tambah") {
				url = link+'jenjang_studi/InsertData';
			}else{
				url = link+'jenjang_studi/UpdateData';
			}
			if (nama_jenjang_studi_status.val() != 1) {
				$('[name=nama_jenjang_studi]').parent().parent().addClass('has-error');
			}else{
				$('[name=nama_jenjang_studi]').parent().parent().removeClass('has-error');
				result += '1';
			}
			if (result == "1") {
				l.start();
	...
	function ResetInputAja() {
		...
		$('#form')[0].reset();
		$('#nama_jenjang_studi_info').text("*Jenjang Studi tidak boleh kosong");		
	}


	function CekNamaJenjangStudi() {
		delay(function(){
			$.ajax({
				url: link+'jenjang_studi/CekNamaJenjangStudi',
				type: 'post',
				data: {nama_jenjang_studi: nama_jenjang_studi.val(), nama_jenjang_studi_lama:nama_jenjang_studi_lama.val()},
				dataType: 'json',
				success: function(data){
					CekIsToken(data.notif);
					$('#nama_jenjang_studi_info').text(data.value);
					if (data.value == '*Jenjang Studi tersedia') {
						$('[name=nama_jenjang_studi_status]').val(1);
						$('[name=nama_jenjang_studi]').parent().parent().removeClass('has-error');
					}else{
						$('[name=nama_jenjang_studi_status]').val(0);
						$('[name=nama_jenjang_studi]').parent().parent().addClass('has-error');
					}
				},
				error: function(){
					notify("Error System", "Gagal Cek Nama Jenjang Studi", "error");
				}
			});
		}, 500);
	}
	...
	function InputStatusHidden(status) {
		nama_jenjang_studi_status.val(status);
	}	
	...
	function EditData(kode){
		...
		ResetInput();
		$.ajax({
			url: link+'jenjang_studi/EditData/'+kode,
			type: 'post',
			dataType: 'json',
			success:function(data){
				CekIsToken(data.notif);
				if (data.notif == 1) {
					...
					id.val(data.data.id);
					nama_jenjang_studi.val(data.data.nama_jenjang_studi);
					nama_jenjang_studi_lama.val(data.data.nama_jenjang_studi);
		...
		$.ajax({
			...
			success:function(data){
				...
				if (data.notif == 1) {
					...
					$('.nama_jenjang_studi').text(data.data.nama_jenjang_studi);
	...
	function DeleteData(kode) {
		swal({
			title: "Hapus data ?",
			text: "Menghapus 1 data jenjang studi",
			type: "warning",
		...
		function(isConfirm) {
			if (isConfirm) {
				setTimeout(function(){
					$.ajax({
						url:link+'jenjang_studi/DeleteData/'+kode,
						type:'post',
						async: false,
						dataType:'json',
						success: function(data){
	...
	function ResetData() {
		var row ;
		$.ajax({
			url:link+'jenjang_studi/CountTable',
			async:false,
			type:'post',
			dataType:'json',
			success:function(data){
		...
		if (row > 1) {
			swal({
				title: "Reset data ?",
				text: "Menghapus semua data jenjang studi ",
				type: "warning",
				showCancelButton: true,
				confirmButtonClass: "btn-warning",
				confirmButtonText: "Ok",
			...
			function(isConfirm) {
				if (isConfirm) {
					setTimeout(function(){
						$.ajax({
							url:link+'jenjang_studi/ResetData',
							type:'post',
							async:false,
							dataType:'json',
							success: function(data){
