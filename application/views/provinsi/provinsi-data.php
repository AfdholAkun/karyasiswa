	...
	<div class="row">
		<div class="panel panel-default">
			<div class="panel-heading" >
				<span style="font-size: 11pt">Data Provinsi
				</span>
			</div>
			...
			<form action="#" id="form" class="form-horizontal">
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<div class="col-md-9">
							<input type="text" name="kode" class="form-control" required maxlength="10" onkeyup="CekKodeProvinsi()">
...
var SaveMethod = "Tambah";
	...
	var nama_provinsi = $('[name=nama_provinsi]');	
	var kode = $('[name=kode]');	
	$(document).ready(function() {
		$('#form').on("submit", function(e){
			...
			if (SaveMethod == "Tambah") {
				url = link+'provinsi/InsertData';
			}else{
				url = link+'provinsi/UpdateData';
			}
	...
	function CekKodeProvinsi() {
		delay(function(){
			$.ajax({
				url: link+'provinsi/CekKodeProvinsi',
				type: 'post',
				data: {kode: kode.val(), kode_lama:kode_lama.val()},
				dataType: 'json',
				success: function(data){
					CekIsToken(data.notif);
				...
				error: function(){
					notify("Error System", "Gagal Cek Kode Provinsi", "error");
				}
			});
		}, 300);
	}
	...
	function EditData(kode){
		...
		ResetInput();
		$.ajax({
			url: link+'provinsi/EditData/'+kode,
			type: 'post',
			dataType: 'json',
			success:function(data){
				CekIsToken(data.notif);
	...
	function DeleteData(kode) {
		swal({
			title: "Hapus data ?",
			text: "Menghapus 1 data provinsi",
			type: "warning",
		...
		function(isConfirm) {
			if (isConfirm) {
				setTimeout(function(){
					$.ajax({
						url:link+'provinsi/DeleteData/'+kode,
						type:'post',
						async: false,
						dataType:'json',
						success: function(data){
	...
	function ResetData() {
		...
		if (row > 1) {
			swal({
				title: "Reset data ?",
				text: "Menghapus semua data provinsi ",
				type: "warning",
				showCancelButton: true,
				confirmButtonClass: "btn-warning",
				confirmButtonText: "Ok",
